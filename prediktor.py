...
GitHub Integration • 8 Metode Analisis • Export TXT
"""
...
# ========== FUNGSI FETCH DATA ==========
def fetch_github_csv(market_key):    file_info = MARKET_FILES.get(market_key)
    if not file_info:
        return None
...
def calc6(data):
    ...
    for digit in range(10):
        sc = 0        max_f = max(freq.values()) or 1
...
def calc3(data):
    ...
    for idx, num in enumerate(data):        if len(num) != 4:
            continue
        for p in [1, 2, 3]:
            d = int(num[p])
            freq[d] += 1
...
def gen3d(f2, top3):
    if not f2 or not top3:
        return []
    res, seen = [], set()
    for s in f2:        a, b = int(s[0]), int(s[1])
        for x in top3:
...
def main():
    ...
    if filt:
        ...
        print(f"\n✂️ Setelah Filter: {len(f_c2)} dari {len(c2)}")
        if len(f_c2) > 0:
            print("  " + " ".join(f_c2))
    ...
    print("\n🔷 Selesai. Jalankan lagi untuk pasaran lain.")
