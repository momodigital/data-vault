...
def calc6(data):
    ...
    for num in data:  # ✅ SUDAH DIPERBAIKI
        if len(num) != 4:
            continue
        for j in range(4):
            d = int(num[j])
...
def main():
    ...
    if save == 'y':
        fname = f"prediktor_{choice}_{datetime.now().strftime('%H%M%S')}.txt"        path = '/sdcard/Download/' + fname if os.path.exists('/sdcard') else fname
